...
# Find all columns generated by your user-item matrix
product_cols = [col for col in final_customer_profile.columns if col.startswith('HAS_PROD_')]

# Features are everything else (excluding IDs)
...
# We calculate the correlation of all features against the product flags
print("Calculating feature correlations...")
corr_matrix = final_customer_profile[feature_cols + product_cols].corr()

# Isolate only the intersections between Features (rows) and Products (columns)
...
for prod in product_cols:
    ...
    # Sort correlations for this specific product
    sorted_corrs = prod_corr[prod].dropna().sort_values(ascending=False)
    
    print("  Top Positive Drivers (High values increase likelihood of ownership):")
    print(sorted_corrs.head(3).to_string())
    
    print("\n  Top Negative Drivers (High values decrease likelihood of ownership):")
    print(sorted_corrs.tail(3).sort_values(ascending=True).to_string())

# 4. Generate & Save a Correlation Heatmap
plt.figure(figsize=(14, 10))
# Using a heatmap to visualize the product_corr matrix
...
heatmap_filename = 'product_feature_correlation_heatmap.png'
plt.savefig(heatmap_filename, dpi=300)
print(f"\n[Saved] Correlation heatmap saved as '{heatmap_filename}'")
plt.close()

# 5. Product Level Summary (Averages)
print("\n--- Average Customer Profile per Product ---")
# Define a subset of key metrics to summarize
...
summary_list = []
for prod in product_cols:
    # Get customers who own this product
...
if summary_list:
    ...
    summary_filename = "product_profile_summary.csv"
    summary_df.to_csv(summary_filename, index=False)
    print(f"\n[Saved] Product summary saved as '{summary_filename}'")